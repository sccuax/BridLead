...
/// <reference path="./collection-list-settings.d.ts" />
...
/**
 * A media query (breakpoint) configured on the current site.
 *
 * Webflow supports up to 7 media queries: `tiny`, `small`, `medium`, `main`,
 * `large`, `xl`, `xxl`. Sites start with 4 (`main`, `medium`, `small`, `tiny`)
 * and users can add up to 3 more (`large`, `xl`, `xxl`) via the Designer.
 * Additions are permanent.
 */
interface MediaQuery {
  /** The media query identifier — one of `'tiny' | 'small' | 'medium' | 'main' | 'large' | 'xl' | 'xxl'`. */
  id: BreakpointId;
  /** User-facing label, e.g. `"Desktop"`, `"Tablet"`, `"Mobile"`, or `"1280px and up"`. */
  name: string;
  /** Minimum width in pixels for the media query, or `null` when unbounded below. */
  minWidth: number | null;
  /** Maximum width in pixels for the media query, or `null` when unbounded above. */
  maxWidth: number | null;
  /** True when this media query is the cascade base for the site (today: `main`). */
  isBase: boolean;
}
...
interface SharedApi {
  ...
  /**
   * Get the list of media queries (breakpoints) configured for the current site.
   *
   * Ordered by **cascade specificity** — the same order Webflow's style cascade
   * walks: the base (`main`) first, then ascending min-widths (`large`, `xl`,
   * `xxl`), then descending max-widths (`medium`, `small`, `tiny`). This is the
   * order partners want when reasoning about which styles apply at a given
   * breakpoint; it is NOT a strict descending sort by viewport width.
   *
   * Sites start with 4 (`main`, `medium`, `small`, `tiny`). Users can add up to
   * 3 more (`large`, `xl`, `xxl`) via the Designer; additions are permanent.
   * @returns A Promise that resolves to an array of `MediaQuery` objects.
   * @example
   * ```ts
   * const all = await webflow.getAllMediaQueries();
   * for (const mq of all) {
   *   console.log(mq.id, mq.name, mq.isBase);
   * }
   * ```
   */
  getAllMediaQueries(): Promise<MediaQuery[]>;
  /**
  ...
  registerComponent(
    name: string,
    root: AnyElement | ElementPreset<AnyElement> | AnyComponent
  ): Promise<Component>;
  /**
   ...
   * ```
   */
  unregisterComponent(component: AnyComponent): Promise<null>;
  /**
   ...
   * ```
   */
  getAllComponents(): Promise<Array<AnyComponent>>;
  /**
   ...
   * ```
   */
  getCurrentComponent(): Promise<AnyComponent | null>;
  /**
   ...
   * ```
   */
  getComponent(id: ComponentId): Promise<AnyComponent>;
  /**
   * Get a Component by its display name.
   * Throws when the matched component is a non-code library component.
   * Use a library-scoped overload to retrieve direct library components.
   ...
   * ```
   */
  getComponentByName(name: string): Promise<AnyComponent>;
  /**
   * Get a Component by its group and display name.
   * If the first argument matches an installed library ID, the lookup is
   * library-scoped. Otherwise, it is treated as a site component group lookup.
   * Throws when a site component group lookup matches a non-code library component.
   * @param groupOrLibraryId - The group name the site component belongs to, or an installed library ID.
   * @param name - The display name of the component.
   * @example
   * ```ts
   * const component = await webflow.getComponentByName('Marketing', 'Hero');
   * const libraryComponent = await webflow.getComponentByName(libraryId, 'Hero');
   * ```
   */
  getComponentByName(
    groupOrLibraryId: string,
    name: string
  ): Promise<AnyComponent>;
  /**
   * Get a library Component by its library ID, group, and display name.
   * @param libraryId - The installed library ID.
   * @param group - The group name the library component belongs to.
   * @param name - The display name of the component.
   * @example
   * ```ts
   * const component = await webflow.getComponentByName(libraryId, 'Marketing', 'Hero');
   * ```
   */
  getComponentByName(
    libraryId: string,
   ...
   * ```
   */
  openCanvas(reference: AnyComponent | ComponentElement | Page): Promise<void>;
  /**
