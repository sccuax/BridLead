...
interface Style {
  ...
  /**
   * The name of this Style as it appears in the Designer's Styles panel
   * (e.g. `'button'`, `'is-primary'`, `'h1'`).
   *
   ...
   * @example
   * ```ts
   * const style = await webflow.getStyleByName('button');
   * console.log(style.name); // 'button'
   * ```
   */
  readonly name: string;

  /**
   * The origin of this Style — whether it was created on the current site
   * or imported from a library.
   *
   * - `'site'`    — the style was created on the current site
   * - `'library'` — the style originated from a Shared Library or
   *                  AI Site Generation
   *
   * Synchronous: the value is hydrated when the Style is first returned
   * from the API and never changes for that instance.
   ...
   * const siteStyles = styles.filter((s) => s.source === 'site');
   * const libraryStyles = styles.filter((s) => s.source === 'library');
   * ```
   */
  readonly source: StyleSource;

  /**
   * The kind of Style — class, combo, HTML tag, element-scoped style, or
   * descendant selector.
   *
   * - `'global'`     — a standalone class (`.my-class`)
   * - `'combo'`      — a class combined with a parent class (`.parent.child`)
   * - `'tag'`        — an HTML tag style (`h1`, `p`, `a`)
   * - `'element'`    — a style scoped to a single element instance
   * - `'descendant'` — a descendant selector (`.parent .child`)
   *
   * Synchronous: the value is hydrated when the Style is first returned
   * from the API and never changes for that instance.
   *
   ...
   * ```ts
   * const styles = await webflow.getAllStyles();
   * const globalClasses = styles.filter((s) => s.type === 'global');
   * const comboClasses = styles.filter((s) => s.type === 'combo');
   * ```
   */
...
/**
 * Public categorization of a Style as it appears in the Designer's Styles
 * panel. See `Style.getType()` for the meaning of each value.
 */
type StyleType = 'global' | 'combo' | 'tag' | 'element' | 'descendant';

/**
 * Origin of a Style — whether it was created on the current site or
 * imported from a library (Shared Library or AI Site Generation).
 * See `Style.source` for details.
 */
type StyleSource = 'site' | 'library';
