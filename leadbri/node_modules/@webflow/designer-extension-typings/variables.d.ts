...
interface VariableCollection {
  ...
  setName(newName: string): Promise<null>;
  /**
   * Moves this variable collection to appear immediately after the target collection
   * in the Variables panel.
   * @param target - The variable collection to position after.
   * @returns A Promise that resolves once the move is successfully applied.
   * @example
   * ```ts
   * const [a, b, c] = await webflow.getAllVariableCollections();
   * await a.moveAfter(c); // Panel order: b, c, a
   * ```
   */
  moveAfter(target: VariableCollection): Promise<null>;
  /**
   * Moves this variable collection to appear immediately before the target collection
   * in the Variables panel.
   * @param target - The variable collection to position before.
   * @returns A Promise that resolves once the move is successfully applied.
   * @example
   * ```ts
   * const [a, b, c] = await webflow.getAllVariableCollections();
   * await c.moveBefore(a); // Panel order: c, a, b
   * ```
   */
  moveBefore(target: VariableCollection): Promise<null>;
  /**
   * Creates a new variable mode.
   *
   * By default the mode is created as a **manual** mode. Pass a `breakpointId`
   * to create an **automatic** mode that is applied to the site whenever the
   * named breakpoint is active.
   *
   * Within a single collection, only one mode (including the implicit base mode)
   * may own a given breakpoint. Passing a breakpoint that's already owned, or
   * a breakpoint that isn't enabled on the site, will reject the promise.
   *
   * @param name - The desired name of the variable mode.
   * @param breakpointId - Optional breakpoint id to drive the mode automatically.
   * @returns A Promise that resolves into the new variable mode.
   * @example
   * ```ts
   * const collection = await webflow.getDefaultVariableCollection();
   * const manual = await collection.createVariableMode('Compact');
   * const tablet = await collection.createVariableMode('Tablet', 'medium');
   * ```
   */
  createVariableMode(
    name: string,
...
interface VariableMode {
  ...
  /**
   * The id of the variable collection this mode belongs to. Carried alongside
   * `id` so the mode can be unambiguously referenced — modes addressed by id
   * alone are not unique across collections (e.g. the base mode shares a
   * single reserved id between every collection).
   */
  readonly variableCollectionId: VariableCollectionId;
  /**
  ...
  /**
   * Gets the breakpoint that drives this mode automatically, if any.
   *
   * @returns A Promise resolving to a `BreakpointId` for automatic modes,
   *   or `null` for manual modes.
   * @example
   * ```ts
   * const mode = await collection.getVariableModeByName('Tablet');
   * const bp = await mode.getBreakpoint();
   * if (bp != null) console.log('Automatic on', bp);
   * ```
   */
  getBreakpoint(): Promise<BreakpointId | null>;
  /**
   * Sets the breakpoint that drives this mode automatically.
   *
   * Pass a `BreakpointId` to convert a manual mode into an automatic mode,
   * or to move an automatic mode to a different breakpoint. Pass `null` to
   * convert an automatic mode back into a manual mode.
   *
   * Rejects if another mode in the same collection already owns the
   * requested breakpoint, or if the breakpoint isn't enabled on the site.
   *
   * @param breakpointId - The breakpoint to bind, or `null` to demote to manual.
   * @returns A Promise that resolves once the breakpoint association has been persisted.
   * @example
   * ```ts
   * const mode = await collection.getVariableModeByName('Tablet');
   * await mode.setBreakpoint('medium'); // promote to automatic
   * await mode.setBreakpoint(null);     // demote to manual
   * ```
   */
  setBreakpoint(breakpointId: BreakpointId | null): Promise<null>;
}
