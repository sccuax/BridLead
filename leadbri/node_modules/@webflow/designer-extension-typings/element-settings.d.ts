...
interface CollectionListElementSetting {
  valueType: 'collectionListSetting';
  canBind: false;
  value: CollectionListSettingValue;
  resolvedValue?: never;
  display: SettingDisplay;
}

type SearchSettingsResult = ElementSetting | CollectionListElementSetting;

type SearchSettingValueType =
  | ElementSetting['valueType']
  | CollectionListElementSetting['valueType'];

interface SearchSettingsOptions {
  /** Filter to settings that produce a specific value type (e.g., "string", "image", "collectionListSetting") */
  valueType?: SearchSettingValueType;
...
interface UnsupportedElementSettingValue {
  sourceType: 'unsupported';
  /** The setting contains inline dynamic bindings and cannot be represented as editable static text. */
  reason: 'containsBindings';
}
...
interface ElementSettingSummaries {
  [key: string]:
    | ResolvedValue
    | BindingValue
    | UnsupportedElementSettingValue
    | ElementAttribute[]
    | null;
}
